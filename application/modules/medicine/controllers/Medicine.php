...
class Medicine extends MX_Controller {
    ...
    function getMedicineExpireAlertList() {
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['medicines'] = $this->medicine_model->getMedicineExpireAlertBysearch($search);
            } else {
                $data['medicines'] = $this->medicine_model->getMedicineExpireAlert();
            }
        } else {
            if (!empty($search)) {
                $data['medicines'] = $this->medicine_model->getMedicineByLimitExpireAlertBySearch($limit, $start, $search);
            } else {
                $data['medicines'] = $this->medicine_model->getMedicineExpireAlertByLimit($limit, $start);
            }
        }
        //  $data['appointments'] = $this->appointment_model->getAppointment();
        $i = 0;
